"""
Modificacion de aeon/classification/deep_learning/_inception_time.py
...
class InceptionTimeClassifier(BaseClassifier):
    ...
    def __init__(
        ...
        self.validation_split = validation_split # ADDED
    ...
    def _fit(self, X, y):
        ...
        for n in range(0, self.n_classifiers):
            cls = IndividualInceptionClassifier(
                ...
                validation_split=self.validation_split,  # ADDED
    ...
    def _fit(self, X, y):
        ...
        if self.validation_split > 0:
            X_train, X_val, y_train_orig, y_val_orig = train_test_split(
                ...
                stratify=y  # Esto garantiza partición estratificada
            )
            # Convertir a one-hot las etiquetas divididas
            y_train = self.convert_y_to_keras(y_train_orig)
            y_val = self.convert_y_to_keras(y_val_orig)
        ...
        if self.callbacks is None: # ADDED and modified
            self.callbacks_ = [
                ...
                # AÑADIR EarlyStopping SI HAY VALIDACIÓN
                *([tf.keras.callbacks.EarlyStopping(
                    monitor="val_loss",
                    patience=100,
                    restore_best_weights=True
                )] if self.validation_split > 0 else [])
            ]
        else:
        ...
        ## === ADDED ===
        if self.validation_split > 0 and hasattr(self.history, 'history'):
            # Encuentra el epoch con la menor val_loss
            val_losses = self.history.history['val_loss']
            self.best_epoch_ = np.argmin(val_losses) + 1  # +1 porque epochs empiezan en 1
            self.best_val_loss_ = np.min(val_losses)
            print(f"    Best epoch: {self.best_epoch_} (val_loss: {self.best_val_loss_:.4f})")
        else:
        ...
        try:
            ...
            self.model_ = tf.keras.models.load_model(model_path, compile=False)
            ...
            print(f"    Warning: Error loading model, using training model copy: {e}")
