...
class MUSE(BaseClassifier): 
    ...
    def __init__(
        ...
        self.estimator = estimator  # ADDED
    ...
    def _fit(self, X, y):
        ...
        if self.estimator is not None:
            ...
            self.clf = _clone_estimator(self.estimator, self.random_state)
        else:
    ...
    def _predict_proba(self, X) -> np.ndarray:
        ...
        if callable(m):
            ...
            # Create probability array with 1s at predicted class positions
            dists = np.zeros((bag.shape[0], len(self.classes_)))
            preds = self.clf.predict(bag)
            for i in range(0, bag.shape[0]):
