...
import numpy as np
from ml_edm.classification import ChronologicalClassifiers, EarlyClassifier
...
def load_dataset(dataset_name, split, z_normalize=False):
    ...
    if split != "default":
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X_, y_, train_size=split, random_state=44, stratify=y_
        )
...
def _fit_chrono_clf(X, y, name, base_classifier, feature_extraction, params, alpha=None):
    ...
    X_clf, X_trigger, y_clf, y_trigger = train_test_split(
        X, y, test_size=params["val_ratio_trigger"], random_state=params["random_state"], stratify=y
    )
    if params["LOADPATH"]:  # load already fitted classifiers
        if feature_extraction:
            path = os.path.join(
                params["LOADPATH"],
                name,
                f"{type(base_classifier).__name__}",
                feature_extraction["method"],
                "classifiers.pkl",
            )
        else:
            path = os.path.join(
                params["LOADPATH"], name, f"{type(base_classifier).__name__}", "classifiers.pkl"
            )
        ...
        chrono_clf = DeepChronologicalClassifier(model=elects, **params["classifiers"]["Elects"])
        chrono_clf.fit(X, y)
    else:
    ...
    if params["SAVEPATH_clf"] and not params["LOADPATH"]:
        clf_name = (
            ...
            if isinstance(chrono_clf, ChronologicalClassifiers)
            else chrono_clf.model._get_name()
        )
        if feature_extraction:
            path = os.path.join(
                params["SAVEPATH_clf"], name, f"{clf_name}", feature_extraction["method"]
            )
        else:
...
def _fit_early_classifier(
    X, y, name, chrono_clf, trigger_model, alpha, n_classes, params, features
):
    def delay_cost(t):
        inflexion_point = 0
    ...
    cost_matrices = CostMatrices(
        chrono_clf.models_input_lengths,
        n_classes,
        alpha=alpha,
        delay_cost=None,
        missclf_cost=misclf_cost,
    )
    ...
    if params["SAVEPATH_early_clf"]:
        trigg_name = (
            type(chrono_clf.base_classifier).__name__
            if isinstance(chrono_clf, ChronologicalClassifiers)
            else chrono_clf.model._get_name()
        )
        if features:
            pp = os.path.join(
                params["SAVEPATH_early_clf"],
                name,
                f"{trigg_name}",
                features["method"],
                f"alpha_{str(alpha)}",
            ...
            pp = os.path.join(
                params["SAVEPATH_early_clf"], name, f"{trigg_name}", f"alpha_{str(alpha)}"
            )
...
def get_output_metrics(early_clf, X, y, compute_post=False, dict_post_cache=None):
    ...
    metrics_additional = {
        "mean_t_star": np.mean(metrics["t_star"]),
    ...
    metrics_additional.update(
        {
            "mean_diff_t_post_pred_star": np.mean(metrics_post["t_post_pred"] - metrics["t_star"]),
    ...
    if early_clf.trigger_model.require_classifiers:  # if use probabilistic classifiers collection
        ...
        metrics_additional.update(
            {
                "mean_t_post_proba": np.mean(metrics_post["t_post_proba"]),
        ...
        metrics_additional.update(
            {
                "mean_diff_t_post_proba_star": np.mean(
                    metrics_post["t_post_proba"] - metrics["t_star"]
                ),
                "std_diff_t_post_proba_star": np.std(
                    metrics_post["t_post_proba"] - metrics["t_star"]
                ),
                "mean_diff_f_star_post_proba": np.mean(
                    metrics["f_star"] - metrics_post["f_post_proba"]
                ),
                "std_diff_f_star_post_proba": np.std(
                    metrics["f_star"] - metrics_post["f_post_proba"]
                ),
            }
        )
...
def train_for_one_alpha(alpha, params, prefit_cost_unaware=False):
    ...
    metrics_alpha = {
        k1: {k2: copy.deepcopy(dict_data) for k2, _ in copy.deepcopy(dict_clf).items()}
    ...
    for dataset in pbar_data:
        ...
        for idx, clf in enumerate(params["classifiers"].keys()):
            ...
            chrono_clf, X_trigger, y_trigger = _fit_chrono_clf(
                data["X_train"],
                data["y_train"],
                dataset,
                base_clf,
            ...
            if features_extractor:
                try:
                    path = os.path.join(
                        features_extractor["path"],
                        dataset,
                        type(chrono_clf.base_classifier).__name__,
                        features_extractor["method"],
                    )
            ...
            for i, trigger in enumerate(params["trigger_models"].keys()):
                ...
                if (
                    prefit_cost_unaware
                    and (trigger in ["ecdire", "edsc", "ects"])
                    and params["SAVEPATH_early_clf"]
                ):
                    if trigger == "ecdire":
                        if features_extractor:
                    ...
                    def delay_cost(t):
                        inflexion_point = 0
                        return np.exp(
                            ((t / data["X_train"].shape[1]) - inflexion_point) * np.log(100)
                        )
                    ...
                    early_clf.cost_matrices = CostMatrices(
                        chrono_clf.models_input_lengths,
                        n_classes,
                        alpha=alpha,
                    ...
                    early_clf = _fit_early_classifier(
                        X_trigger,
                        y_trigger,
                        dataset,
                        chrono_clf,
                ...
                if features_extractor:
                    if features_paths:
                        early_clf.chronological_classifiers.feature_extraction = features_paths[
                            "test"
                        ]
                        if trigger == "ecdire":
                            early_clf.new_chronological_classifiers.feature_extraction = (
                                features_paths["test"]
                            )

                metrics, post, add = get_output_metrics(
                    early_clf,
                    data["X_test"],
                    data["y_test"],
                    compute_post=get_post,
                    dict_post_cache=past_post,
                )
                past_post = post
    ...
    with open(os.path.join(tmp_dir, "res.json"), "w") as tmp_file:
...
def compute_baselines(alphas, params):
    ...
    for dataset in params["datasets"]:
        ...
        for clf in params["classifiers"].keys():
            ...
            chrono_clf, X_trigger, y_trigger = _fit_chrono_clf(
                data["X_train"], data["y_train"], dataset, base_clf, features_extractor, params
            )
    ...
    for alpha in alphas:
        ...
        with open(
            os.path.join(path, "baselines_ridgeclassifiercv_minirocket.json"), "w"
        ) as tmp_file:
...
if __name__ == "__main__":
    ...
    if params["trigger_models"] == "all":
        all_trigger_models = [
            t[0]
            for t in getmembers(sys.modules["trigger_models"], isclass)
            if t[1].__module__ == "trigger_models"
        ]
        all_trigger_models += [
            t[0]
            for t in getmembers(sys.modules["trigger_models_full"], isclass)
